...
export class StraightLinkRenderer implements IRenderer
{
    constructor() {
    ...
    bindContext(gl: WebGLRenderingContext, compiler: Rendering.IShaderCompiler) {
        ...
        gl.bindBuffer(gl.ARRAY_BUFFER, this.instanced_buffer);
        gl.bufferData(gl.ARRAY_BUFFER, this.instanced_data.attributes, gl.DYNAMIC_DRAW);
    ...
    addObject(object: any): void
    {
        ...
        if (!this.offsets.has(link)) {
            ...
            const new_attributes = StraightLinkRenderer.makeAttributesArray(link);
    ...
    sortObjects() : void
    {
        ...
        for(let i = 0; i < this.objects.length; i++)
        {
            ...
            const old_offset = this.offsets.get(correct_object);
            for(let j = 0; j < FLOATS_PER_INSTANCE; j++)
                new_buf[i * FLOATS_PER_INSTANCE + j] = this.instanced_data.attributes[old_offset * FLOATS_PER_INSTANCE + j];
            this.offsets.set(correct_object, i);
        }
        this.instanced_data.attributes = new_buf;
        this.objects = sorted_objects;
        this.gl.bindBuffer(this.gl.ARRAY_BUFFER, this.instanced_buffer);
        this.gl.bufferSubData(this.gl.ARRAY_BUFFER, 0, this.instanced_data.attributes);
        this.gl.bindBuffer(this.gl.ARRAY_BUFFER, null);
    }
    ...
    private shader_source : ShaderProgramSource = {
        ...
        attributes: ["a_vertexPos", "a_from_color", "a_to_color", "a_transform"],
        uniforms: ["u_pv"]
    };
    ...
    private static makeAttributesArray(link: LinkView) : number[]
    {
        ...
        const cos = dx / len;
        const sin = dy / len;
        //const scaled_cos = cos * link.size;
        //const scaled_sin = sin * link.size;
        ...
        return [from_color[0], from_color[1], from_color[2], from_color[3],
                ...
                from.position.x, from.position.y, 1];
    }
}